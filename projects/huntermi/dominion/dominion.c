...
int fullDeckCount(int player, int card, struct gameState *state) {
	...
	{
		if (card == -1 || state->deck[player][i] == card) count++;
	}

	for (i = 0; i < state->handCount[player]; i++)
	{
		if (card == -1 || state->hand[player][i] == card) count++;
	}

	for (i = 0; i < state->discardCount[player]; i++)
	{
		if (card == -1 || state->discard[player][i] == card) count++;
	}
...
int scoreFor (int player, struct gameState *state) {
	...
	for (i = 0; i < state->handCount[player]; i++)
	{
		...
		if (state->hand[player][i] == gardens)
			score = score + ( fullDeckCount(player, -1, state) / 10 );
	}
...
int playMine(struct gameState *state, int handPos, int choice1, int choice2){
	...
	if (choice2 > treasure_map || choice2 < curse)//should be gold and copper
	{
		return -1;
	}
	...
	gainCard(choice1, state, 2, currentPlayer);//should be choice2
